...
(function() {
  ...
  Game["new"] = function() {
    var animateTile, curTileIndex, i, shuffTiles, t, tile, tiles, _i, _j, _ref, _ref1;
    ...
    tiles = [];
    curTileIndex = 0;
    for (i = _i = 0, _ref = (Game.width * Game.height) / 2 - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
      tiles.push(new root.Tile(Game.tileClasses[curTileIndex]));
      tiles.push(new root.Tile(Game.tileClasses[curTileIndex]));
      curTileIndex += 1;
      if (curTileIndex === Game.tileClasses.length) {
        curTileIndex = 0;
      }
    }
    ...
    for (i = _j = 0, _ref1 = shuffTiles.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
      tile = shuffTiles[i];
  ...
  Game.draw = function() {
    var elapsed, fraction, pad, renderWidth, tile, time, x, y, _i, _ref, _results;
    time = (new Date).getTime();
    Game.ctx.clearRect(0, 0, Game.canvas.width, Game.canvas.height);
    pad = Game.tilePad;
    _results = [];
    for (x = _i = 0, _ref = Game.width - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; x = 0 <= _ref ? ++_i : --_i) {
      _results.push((function() {
        var _j, _ref1, _results1;
        _results1 = [];
        for (y = _j = 0, _ref1 = Game.height - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; y = 0 <= _ref1 ? ++_j : --_j) {
